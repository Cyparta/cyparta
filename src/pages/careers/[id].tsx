...
const CareerDetails = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      name: Yup.string().required(lang==="en" ? "Please enter name" : "الرجاء ادخل اسمك"),
      development: Yup.string(),
      phone_number: Yup.number().required(lang==="en" ? "Please enter a phone number" : "الرجاء ادخل رقمك"),
      summary: Yup.string().required(lang==="en" ? "Please enter a summary" : "الرجاء إدخال ملخص"),
      email: Yup.string()
        .email("Invalid email address")
        .required(lang==="en" ? "Please enter a vaild email" : "الرجاء إدخال بريدك الالكتروني"),
      cv: Yup.mixed().required( lang==="en" ? "A PDF file is required" : "الرجاء ادخال ملف"),
    }),
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      ...
      try {
        const response = await axios.post(
          `${BASEURL}apis/careers/${careerID}/cvs/`,
          formData,
          {
            headers: {
      ...
      <Container>
          ...
          >
            <Box>
              ...
              >
                ...
                <p style={{ fontSize: "14px", color: "#727071" }}>
                  Posted{" "}
                  {career.posted_at !== null ? career.posted_at : "1 month ago"}
                </p>
              </Box>
              <Box
                sx={{
                ...
                {isShare && (
                  ...
                  >
                    <Box sx={{ display: "flex", gap: "20px", }}>
                      <FacebookShareButton
                        url={imageLink}
                        onClick={() => setIsShare((old) => !old)}
                      >
                        <FacebookOutlinedIcon
                          sx={{ color: "rgba(59, 89, 152, 1)" }}
                        />
                      </FacebookShareButton>
                      <TwitterShareButton
                        url={imageLink}
        ...
        >
          <Typography sx={{ color: "rgba(114, 112, 113, 1)" }}>
            ...
            {career.jop_description && (
              <div
                dangerouslySetInnerHTML={{ __html: career.jop_description }}
              ...
              >
                ...
                <Box sx={{ display: "flex", justifyContent: "center" }}>
                  <MainButton type="submit">
                    {formik.isSubmitting ? "...loading" : "Apply"}
                  </MainButton>
                </Box>
              </Box>
            </form>
